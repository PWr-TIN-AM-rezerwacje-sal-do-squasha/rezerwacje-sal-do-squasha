class Facility < ApplicationRecord
  ...
  validates :localNumber, presence: true, uniqueness: false, length: { maximum: 5 }, format: { with: /\A[0-9]+\z/, message: "only digits" }
  validates :postalCode, presence: true, uniqueness: false, format: { with: /\A[0-9]{2}-[0-9]{3}\z/, message: "must look like 00-000" }
  validates :openingTime, presence: true, uniqueness: false
  validates :closingTime, presence: true, uniqueness: false, comparison: { greater_than: :openingTime }
